"""Agent lifecycle loop - continuous retry with context reset."""
...
from runtime.process import TerminalManager, AgentProcess
...
class AgentLifecycleLoop:
    ...
    def __init__(
        ...
        self.snapshot_dir = Path(snapshot_dir) if snapshot_dir else repo_dir / ".state" / "snapshots"
    ...
    async def _spawn_fresh_agent(
        ...
        workspace_path = self.workspace_manager.get_sandbox(agent.agent_id)
        if not workspace_path:
            workspace_path = self.workspace_manager.create_sandbox(agent.agent_id)

        self.workspace_manager.inject_claude_md(agent.agent_id, claude_md_content)
...
RalphWiggumLoop = AgentLifecycleLoop
